...
const FolderSettingsDialog = ({ visible, onClose, folderName, currentColor, onRename, onColorChange, onDelete, settings }) => {
  ...
  useEffect(() => {
    if (visible) {
      ...
      setSelectedColor(currentColor || BRAND_COLOR);
      setError('');
    }
    ...
    <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}>
      <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: 'rgba(0,0,0,0.5)' }}>
        <View style={{ backgroundColor: 'white', padding: 20, borderRadius: 10, width: width - 40 }}>
          ...
          <TouchableOpacity onPress={onDelete} style={{ marginTop: 16, padding: 12, backgroundColor: '#FF6B6B', borderRadius: 5, alignItems: 'center' }}>
